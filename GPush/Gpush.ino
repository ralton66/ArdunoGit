...
void setup() {
  ...
  free_heap_before = 1000;
  free_stack_before = 20000;
  Serial.print("Free heap before: ");
  Serial.print( free_heap_before);
  Serial.print("unmodified stack   = ");
  Serial.print(free_stack_before);
  ...
  payload = payload_base + "\"" + 1001 + "," + 2001 + "\"}";
  client->POST(url2, host, payload, false);
...
void loop() {
  ...
  if (!flag){
    free_heap_before = 10002;
    free_stack_before = 20001;
    client = new HTTPSRedirect(httpsPort);
    flag = true;
    client->setPrintResponseBody(true);
    client->setContentTypeHeader("application/json");
